...
a = rep(1,24); #remove +1 if nrow is divisible by 25
b = rep(2,24);
c = rep(3,24);
d = rep(4,24);

all_run = data.frame(row.names=1:96)
#if(FALSE) {
for(i in 1:200) {
...
write.table(all_prob, file="gem_probabilities.tsv", row.names=T);
#} #IF FALSE

#pROC
rf_prob = predict(random_forest, validation_new, type = "prob" );
write.table(rf_prob[,2], file="gem_val_probabilities.tsv", row.names=T)
